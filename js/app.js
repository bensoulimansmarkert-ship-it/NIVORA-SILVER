(() => {
  ...
  const card = p => `<article class="product-card"><a href="product.html?id=${encodeURIComponent(p.id)}"><div class="product-img"><span class="tag">925</span></div></a><div class="product-body"><h3>${p.title}</h3><p>★ ${p.rating} · فضة إسترليني</p><div class="price"><strong>${money(p.price)}</strong><button class="mini-btn" data-add="${p.id}">أضف للسلة</button></div></div></article>`;
  ...
  if(pv){
    ...
    pv.innerHTML=`<div class="detail-grid"><div class="detail-image"></div><div class="detail-info"><p class="eyebrow">NIVORA / 925</p><h1>${p.title}</h1><p>★ ${p.rating} · فضة إسترليني 925</p><div class="detail-price">${money(p.price)}</div><p class="desc">${p.description}</p><div class="quantity"><span>الكمية</span><input id="qty" type="number" min="1" value="1"></div><div class="detail-actions"><button class="btn btn-light" id="add-detail">أضف للسلة</button><a class="btn" href="checkout.html">اذهب للدفع</a></div><hr style="border-color:#222;margin:35px 0"><p class="desc">فضة 925 أصلية · تغليف آمن · شحن مباشر من المورد عند تفعيل المصدر · سياسة استرجاع 14 يومًا وفق الشروط.</p></div></div>`;
